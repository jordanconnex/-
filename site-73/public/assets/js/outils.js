...
(function () {
  ...
  let dayKey;
  try {
    let dk = new Intl.DateTimeFormat("en-CA", { timeZone: D.config.fuseau, year: "numeric", month: "2-digit", day: "2-digit" });
    dayKey = function (d) { return dk.format(d); };
  } catch (e) {
    dayKey = function (d) { return d.toISOString().slice(0, 10); };
  }
  let MOIS_LONG = ["janvier", "février", "mars", "avril", "mai", "juin", "juillet", "août", "septembre", "octobre", "novembre", "décembre"];
  ...
  function evenements() {
    let list = $("#evt-list");
    if (!list) return;
    let T = D.typesEvenement;
    let trier = function () { return (D.evenements || []).slice().sort(function (a, b) { return new Date(a.date) - new Date(b.date); }); };
    let all = trier();
    let state = { type: "all" };
    let status = function (e) {
      let t0 = new Date(e.date).getTime(), t1 = t0 + e.duree * 60000, now = Date.now();
      return now < t0 ? "avenir" : now < t1 ? "encours" : "termine";
    };
    let discordText = function (e) {
    ...
    let countdownBlocks = function (ms) {
    ...
    doc.addEventListener("s73:tick", function () {
      ...
      $$("[data-evt-count]", list).forEach(function (el) {
        let e = all.filter(function (x) { return x.id === el.getAttribute("data-evt-count"); })[0];
        if (!e) return;
        let st = status(e);
    ...
    // --- Calendrier
    let cal = $("#evt-cal");
    let first = S.upcoming()[0] || all[all.length - 1] || { date: new Date().toISOString() };
    let viewY = +first.date.slice(0, 4), viewM = +first.date.slice(5, 7) - 1;
    let today = dayKey(new Date());
    let renderCal = function () {
      let byDay = {};
      all.forEach(function (e) { let k = e.date.slice(0, 10); (byDay[k] = byDay[k] || []).push(e); });
      let firstDay = new Date(Date.UTC(viewY, viewM, 1));
      let offset = (firstDay.getUTCDay() + 6) % 7;
      let nDays = new Date(Date.UTC(viewY, viewM + 1, 0)).getUTCDate();
      let cells = "";
      for (let i = 0; i < offset; i++) cells += '<div class="cal__cell is-empty" aria-hidden="true"></div>';
      for (let d = 1; d <= nDays; d++) {
        let key = viewY + "-" + pad(viewM + 1) + "-" + pad(d);
        let evs = byDay[key] || [];
        let cls = "cal__cell" + (key === today ? " is-today" : "") + (evs.length ? " has-evt" : "");
        if (evs.length) {
      ...
      let rest = (offset + nDays) % 7;
      if (rest) for (let r = rest; r < 7; r++) cells += '<div class="cal__cell is-empty" aria-hidden="true"></div>';
      cal.innerHTML =
    ...
    cal.addEventListener("click", function (e) {
      ...
      let d = e.target.closest("[data-day]");
      if (d) {
        let ev = all.filter(function (x) { return x.date.slice(0, 10) === d.getAttribute("data-day"); })[0];
    ...
    // --- Liste
    let filt = $("#evt-filters");
    let keys = ["all"].concat(Object.keys(T)).concat(["planning"]);
    let drawFilters = function () {
      filt.innerHTML = keys.map(function (k) {
        let n = k === "all" ? all.length : k === "planning" ? all.filter(function (e) { return S.isMarked("planning", e.id); }).length : all.filter(function (e) { return e.type === k; }).length;
        if (!n && k !== "planning" && k !== "all") return "";
        let c = k === "all" ? "var(--text-2)" : k === "planning" ? "var(--signal)" : T[k].couleur;
    ...
    let renderList = function () {
      ...
      let up = items.filter(function (e) { return status(e) !== "termine"; });
      let past = items.filter(function (e) { return status(e) === "termine"; }).reverse();
      let card = function (e) {
        let st = status(e), on = S.isMarked("planning", e.id);
        let d = new Date(e.date);
        let parts = S.fmtEvent(e.date).split(" ");
    ...
    list.addEventListener("click", function (e) {
      let p = e.target.closest("[data-plan]");
      if (p) {
        let id = p.getAttribute("data-plan");
        let on = !S.isMarked("planning", id);
      ...
      if (c) {
        let ev = all.filter(function (x) { return x.id === c.getAttribute("data-copy"); })[0];
        S.stat("copies");
  ...
  /* ======================================================================
     ...
     ====================================================================== */
  function protocoles() {
    let panel = $("#proto-panel");
    if (!panel) return;
    let P = D.protocoles;
    let state = { role: P.roles[0].id, niveau: S.getAlert() };
    let roles = $("#proto-roles"), levels = $("#proto-levels");
    ...
    let render = function () {
      ...
      let lv = P.niveaux[state.niveau], a = D.alertes[state.niveau];
      let role = P.roles.filter(function (r) { return r.id === state.role; })[0];
      ...
      let gen = $("[data-gen]", panel), rl = $("[data-role-list]", panel);
      lv.general.forEach(function (t) { let li = doc.createElement("li"); S.redactInto(li, t); gen.appendChild(li); });
      (lv.roles[state.role] || []).forEach(function (t) { let li = doc.createElement("li"); S.redactInto(li, t); rl.appendChild(li); });
    };
    roles.addEventListener("click", function (e) { let b = e.target.closest("[data-role]"); if (b) { state.role = b.getAttribute("data-role"); render(); } });
    levels.addEventListener("click", function (e) { let b = e.target.closest("[data-lvl]"); if (b) { state.niveau = b.getAttribute("data-lvl"); render(); } });
    render();
    ...
    // Alphabet radio
    let pin = $("#proto-phon-input"), pout = $("#proto-phon-out"), pspeak = $("#proto-phon-speak");
    let spell = function (txt) {
    ...
    let renderPhon = function () {
      let words = spell(pin.value);
    ...
    if (pspeak) {
      ...
      pspeak.addEventListener("click", function () {
        let w = spell(pin.value).filter(function (x) { return x !== "·"; });
    ...
    // Annonce générale
    let tpl = $("#pa-tpl"), scp = $("#pa-scp"), zone = $("#pa-zone"), pers = $("#pa-personne"), text = $("#pa-text"), play = $("#pa-play"), copy = $("#pa-copy"), box = $("#pa-console");
    ...
    scp.value = "173";
    let syncZone = function () { let s = S.scpById[scp.value]; if (s && S.zoneById[s.zone]) zone.value = s.zone; };
    syncZone();
    let build = function () {
      let a = D.annonces.filter(function (x) { return x.id === tpl.value; })[0];
      let s = S.scpById[scp.value], z = S.zoneById[zone.value];
    ...
    sync();
    let playing = false;
    let stopPA = function () { playing = false; box.classList.remove("is-live"); play.innerHTML = S.icon.speak + "<span>Diffuser l'annonce</span>"; };
    play.addEventListener("click", function () {
      ...
      setTimeout(function () {
        if (!playing) return;
        let ok = S.speak(build(), { rate: 0.92, pitch: 0.85, onend: stopPA });
        if (!ok) {
  ...
  function laboratoire() {
    ...
    // --- SCP-914
    let obj = $("#m914-obj"), custom = $("#m914-custom"), customWrap = $("#m914-custom-wrap"), regs = $("#m914-reg"), run = $("#m914-run"), out = $("#m914-out"), logEl = $("#m914-log"), copyLog = $("#m914-copy");
    let reg = 2, busy = false, registre = [];
    ...
    let knob = $(".m914__needle", machine);
    let syncReg = function () {
    ...
    regs.addEventListener("click", function (e) { let b = e.target.closest("[data-reg]"); if (b && !busy) { reg = +b.getAttribute("data-reg"); syncReg(); S.sfx("tick"); } });
    machine.addEventListener("click", function (e) { let t = e.target.closest("[data-dial]"); if (t && !busy) { reg = +t.getAttribute("data-dial"); syncReg(); S.sfx("tick"); } });
    obj.addEventListener("change", function () { customWrap.hidden = obj.value !== "autre"; if (obj.value === "autre") custom.focus(); });
    syncReg();
    let objName = function () { return obj.value === "autre" ? (custom.value.trim() || "objet non identifié") : L.objets[+obj.value].nom; };
    let drawLog = function () {
    ...
    run.addEventListener("click", function () {
      ...
      let n = 0;
      let tickT = setInterval(function () { S.tone(n++ % 2 ? 1200 : 900, 0.03, { vol: 0.02, force: S.getSetting("sfx") }); }, 160);
      let name = objName();
      setTimeout(function () {
        clearInterval(tickT);
        let res = S.run914(name, reg);
        busy = false;
        run.disabled = false;
    ...
    let rapLoad = function (v) { Object.keys(R).forEach(function (k) { if (v[k] != null) R[k].value = v[k]; }); };
    rapLoad(rapSaved || RAP_EX);
    let rapPrev = $("#rap-preview");
    let rapData = function () { let v = {}; Object.keys(R).forEach(function (k) { v[k] = R[k].value.trim(); }); return v; };
    let refOf = function (v) { let t = TYPES[v.type]; return t.pre + "-73-" + (v.date || "2026").slice(0, 4) + "-" + pad(U.hash(v.deroul + v.date) % 100); };
    let fmtD = function (v) {
      if (!v.date) return "—";
      let p = v.date.split("T");
      return U.fmtDate(p[0]) + (p[1] ? " · " + p[1].replace(":", " h ") : "");
    };
    let rapRender = function () {
      let v = rapData(), t = TYPES[v.type] || TYPES.incident;
      $("#rap-l1").textContent = t.l1;
      $("#rap-l2").textContent = t.l2;
      let z = S.zoneById[v.zone], s = v.scp ? S.scpById[v.scp] : null, lvl = +v.niveau;
      rapPrev.innerHTML =
    ...
    $("#rap-copy").addEventListener("click", function () {
      let v = rapData(), t = TYPES[v.type], z = S.zoneById[v.zone], s = v.scp ? S.scpById[v.scp] : null;
      let txt = [
    ...
    let view = $("#cre-view"), viewOut = $("#cre-view-out"), crePrev = $("#cre-preview");
    let CRE_EX = {
    ...
    let creLoad = function (v) { Object.keys(C).forEach(function (k) { if (v[k] != null) C[k].value = v[k]; }); };
    creLoad(creSaved || CRE_EX);
    view.value = String(S.getClearance());
    let creData = function () { let v = {}; Object.keys(C).forEach(function (k) { v[k] = C[k].value.trim(); }); return v; };
    let creRender = function () {
      let v = creData(), lvl = +view.value;
      viewOut.textContent = "Niveau " + lvl + " · " + S.habName(lvl);
      let meterTxt = "";
      for (let i = 1; i <= 5; i++) meterTxt += i <= +v.menace ? "■" : "□";
    ...
    $$("[data-insert]").forEach(function (b) {
      b.addEventListener("click", function () {
        let ta = C[b.getAttribute("data-target")] || C.desc;
        let ins = b.getAttribute("data-insert");
        let st = ta.selectionStart || ta.value.length, en = ta.selectionEnd || st;
        let sel = ta.value.slice(st, en);
        let txt = ins === "sup" ? "[DONNÉES SUPPRIMÉES]" : "[[" + ins + "|" + (sel || "texte masqué") + "]]";
    ...
    $("#cre-copy").addEventListener("click", function () {
      let v = creData();
      let sp = function (t) { return t.replace(/\[\[(\d)\|([\s\S]*?)\]\]/g, "||$2||"); };
      let txt = [
  ...
  function entrainement() {
    ...
    // --- Contact visuel
    let statue = $("#g173-statue"), startBtn = $("#g173-start"), lightBtn = $("#g173-light"), msg = $("#g173-msg"), batEl = $("#g173-bat"), timeEl = $("#g173-time"), distEl = $("#g173-dist");
    let G = { running: false, dist: 5, bat: 8, left: 40, dark: false, lit: false, timers: [], iv: null };
    let DIST_M = ["0", "2", "4", "7", "10", "14"];
    let sound = function (f, d, t, v) { S.tone(f, d, { type: t || "square", vol: v || 0.05, force: true }); };
    let hud = function () {
      batEl.innerHTML = "";
      for (let i = 0; i < 8; i++) batEl.innerHTML += '<i class="' + (i < G.bat ? "on" : "") + '"></i>';
    ...
    let say = function (t, cls) { msg.className = "g173__msg" + (cls ? " " + cls : ""); msg.textContent = t; };
    let clear = function () { G.timers.forEach(clearTimeout); G.timers = []; clearInterval(G.iv); };
    let end = function (win) {
      G.running = false;
      clear();
    ...
    let blackout = function () {
      ...
      let win = Math.max(520, 900 - (40 - G.left) * 8);
    ...
    let schedule = function () {
      let base = Math.max(700, 2600 - (40 - G.left) * 40);
    ...
    doc.addEventListener("keydown", function (e) {
      ...
      let space = e.code === "Space" || e.key === " ";
      let enter = e.key === "Enter" && e.target === lightBtn;
    ...
    let pads = $$("#simon .simon__pad"), sStart = $("#simon-start"), sLvl = $("#simon-lvl"), sBest = $("#simon-best"), sMsg = $("#simon-msg");
    let FREQ = [329.6, 392, 523.3, 659.3];
    let seq = [], pos = 0, accept = false, playingSeq = false;
    let best = function () { return S.carnet().stats.simon || 0; };
    let flash = function (i, dur) {
      let p = pads[i];
      p.classList.add("is-on");
      S.tone(FREQ[i], dur / 1000, { type: "triangle", vol: 0.07, force: true });
      setTimeout(function () { p.classList.remove("is-on"); }, dur);
    };
    let showSeq = function () {
      accept = false; playingSeq = true;
      sMsg.textContent = "Mémorise la séquence de verrouillage…";
      let speed = Math.max(260, 620 - seq.length * 28);
  ...
  function carnetPage() {
    ...
    let render = function () {
      let c = S.carnet();
      let n = U.count(c.badges), tot = D.distinctions.length;
      let seenN = D.scp.filter(function (s) { return c.seen[s.id]; }).length;
      let pagesN = S.pages.filter(function (p) { return c.pages[p.id]; }).length;
      let rulesN = D.reglement.filter(function (ch) { return c.rules[ch.id]; }).length;
      let planN = (D.evenements || []).filter(function (e) { return c.planning[e.id]; }).length;
      let st = c.stats;
      let stat = function (v, t, lbl) { return '<div class="stat"><b class="mono"><span data-compter>' + v + "</span>" + (t != null ? "<small>/" + t + "</small>" : "") + "</b><span>" + lbl + "</span></div>"; };
      ...
      root.innerHTML = D.distinctions.map(function (b) {
        let got = c.badges[b.id];
        let hidden = b.secret && !got;
      ...
      $("#carnet-seen").innerHTML = D.scp.map(function (s) {
        let on = c.seen[s.id], fav = c.fav[s.id];
      ...
      let plan = (D.evenements || []).filter(function (e) { return c.planning[e.id]; });
    ...
    let motionTxt = function () {
      let el = $("[data-motion-txt]", setBox);
    ...
    setBox.addEventListener("click", function (e) {
      let mo = e.target.closest("[data-motion]");
      if (mo) {
      ...
      let sw = e.target.closest("[data-set]");
      if (sw) {
        let k = sw.getAttribute("data-set"), on = sw.getAttribute("aria-checked") !== "true";
        S.setSetting(k, on);
        sw.setAttribute("aria-checked", String(on));
        if (k === "sfx" && on) S.sfx("ok");
        return;
      }
      let act = e.target.closest(".setting__act");
