...
(function () {
  ...
  S.staffModule = function () {
    ...
    let etat = null, filtre = { q: "", niveau: "all" }, confirmer = null;
    ...
    let TITRES = { tableau: "Tableau de bord", alerte: "Niveau d'alerte", membres: "Habilitations", communiques: "Communiqués", evenements: "Événements", journal: "Journal des actions" };
    let onglet = S.store.get("s73.console", true) || "tableau";
    if (!TITRES[onglet]) onglet = "tableau";
    let nav = $("#cons-nav");
    let placerInd = function () {
      let b = $('[aria-selected="true"]', nav), ind = $(".cons__ind", nav);
      if (!b || !ind) return;
      let vertical = getComputedStyle(nav).flexDirection === "column";
    ...
    nav.addEventListener("keydown", function (e) {
      let ids = Object.keys(TITRES), i = ids.indexOf(onglet);
      let k = e.key;
    ...
    let renderTableau = function () {
      ...
      let n = { membres: etat.membres.length, communiques: etat.communiques.length, evenements: etat.evenements.filter(function (e) { return new Date(e.date).getTime() + e.duree * 60000 > Date.now(); }).length };
      ...
      let proch = etat.evenements.filter(function (e) { return new Date(e.date).getTime() + e.duree * 60000 > Date.now(); })
